// Command kagent-pi runs the Pi Harness runtime adapter.
package main
...
import (
	...
	"github.com/kagent-dev/kagent/go/harness/pi/internal/adapter"
...
func main() {
	check := flag.Bool("check", false, "validate configuration and Pi version, then exit")
	flag.Parse()
	...
	if err := run(ctx, *check, os.Getenv, os.Environ()); err != nil {
		logger.ErrorContext(ctx, "pi harness stopped", "error", err)
		os.Exit(1)
	}
}
...
func run(ctx context.Context, check bool, getenv func(string) string, environment []string) error {
	...
	if err != nil {
		return fmt.Errorf("configure Pi Harness: %w", err)
	}
	...
	store, err := continuation.New(dataDir+"/adapter", "pi", validateSessionID)
	if err != nil {
		return err
	}
...
func validateSessionID(id string) error {
	if id == "" || len(id) > 256 {
		return fmt.Errorf("invalid Pi session ID length")
	}
	for _, character := range id {
		if unicode.IsControl(character) || unicode.IsSpace(character) {
			return fmt.Errorf("invalid Pi session ID")
		}
	}
	return nil
}
